      ...
      <div className="relative bg-gradient-to-b from-zinc-900 to-zinc-950 border border-white/10 rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <p className="text-xs text-zinc-500 text-center">
            By continuing, you agree to our domain registration terms and Stripe payment processing.
          </p>
        </form>
      </div>
    </div>
...
/**
 * Standalone Purchase Flow with Dialog
 */
interface DomainSearchWithPurchaseProps {
  DomainSearchComponent: React.ComponentType<any>;
  workspaceId: string;
  onPurchaseSuccess?: (domain: string, registrationId: string) => void;
}

export const DomainSearchWithPurchase: React.FC<DomainSearchWithPurchaseProps> = ({
  ...
  const handlePurchaseConfirm = async (data: PurchaseFormData) => {
    ...
    try {
      ...
      const response = await fetch('/api/domains/purchase', {
        ...
        body: JSON.stringify({
          ...
          // stripePaymentIntentId will be added after Stripe checkout
        }),
      });
      ...
      onPurchaseSuccess?.(data.domain, result.data.registrationId);
    } catch (error) {
  ...
  return (
    <>
      <DomainSearchComponent onDomainSelect={handleDomainSelect} workspaceId={workspaceId} />
