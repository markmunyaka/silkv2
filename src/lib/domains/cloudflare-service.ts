...
export class CloudflareHostingService implements IHostingService {
  ...
  async provisionDomain(request: HostingProvisioningRequest): Promise<HostingProvisioningResult> {
    try {
      ...
      return {
        ...
        domain,
        status: 'provisioning',
        sslStatus: customHostname.ssl?.status as 'pending' | 'active' | 'error' || 'pending',
  ...
  async getProvisioningStatus(domain: string): Promise<HostingProvisioningResult> {
    try {
      ...
      if (!zoneId) {
        return { id: 'unknown', domain: normalizedDomain, status: 'failed', error: 'Zone not found' };
      }

      const customHostname = await this.getCustomHostname(zoneId, normalizedDomain);

      if (!customHostname) {
        return { id: 'unknown', domain: normalizedDomain, status: 'pending' };
      }
  ...
  async getDnsValidationRecords(
    domain: string,
  ): Promise<{ recordType: string; name: string; value: string }[]> {
    try {
      const normalizedDomain = normalizeDomain(domain);
      const zoneId = await this.getZoneId(normalizedDomain);
  ...
  async verifyDnsPropagation(domain: string): Promise<{ verified: boolean; details?: Record<string, unknown> }> {
    try {
      ...
      if (!validationRecords || validationRecords.length === 0) {
        return { verified: false, details: { reason: 'No validation records found' } };
      }

      return { verified: true, details: { recordsFound: validationRecords.length } };
    } catch (error) {
  ...
  async getSslCertificateStatus(
    ...
    try {
      ...
      return {
        status: (ssl.status as 'pending' | 'active' | 'error') || 'pending',
      ...
      return { status: 'error', validationErrors: [error instanceof Error ? error.message : 'Unknown error'] };
    }
  }
  ...
  async listProvisionedDomains(
    ...
    try {
      ...
      const response = await withTimeout(
        this.client.get(`/zones/${this.config.zoneName}/custom_hostnames?limit=${limit}&offset=${offset}`),
        15000,
  ...
  private async createCustomHostname(
    ...
    try {
      const response = await withTimeout(
        this.client.post<CloudflareCustomHostnameResponse>(`/zones/${zoneId}/custom_hostnames`, {
          hostname,
  ...
  private async getCustomHostname(
    ...
    try {
      const response = await withTimeout(
        this.client.get<{ success: boolean; result: any[] }>(`/zones/${zoneId}/custom_hostnames?hostname=${hostname}`),
        10000,
  ...
  private mapHostnameStatusToProvisioningStatus(sslStatus?: string): 'pending' | 'provisioning' | 'active' | 'failed' {
    switch (sslStatus) {
      ...
      case 'issuance_timed_out': return 'failed';
